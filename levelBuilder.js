...
level.shrubs.forEach((shrub) => {
  ...
  level.mountains.forEach((mountain) => {
    this.sceneryEntities.push(
      new Mountain(
        mountainsImage, mountain[0], mountain[1], mountain[2], mountain[3]),
    );
  });
...
stock(gameObj){
    // adds to gameobj for collision and physx.
this.sceneryEntities.forEach((entity)=>{
    gameObj.entity.scenery.push(entity);
})
}
render(gameObj){
    gameObj.entity.scenery.forEach((entity)=>{
        // console.log(entity)
